    ...
    for m_path in formulations_path:
        ...
        for p_path in problems_path:
            ...
            for rep in range(REPETITIONS):
                print(
                    Fore.CYAN + f"[{m_path}{p_path}] repetition {rep + 1}/{REPETITIONS}"
                )
        ...
        for rep in range(REPETITIONS):
            ...
            if result["status"] == "timeout":
                ...
                if partial and partial.get("status") == "built":
                    building_time = partial["building_time_us"]
                    m.append(
                        [TIMEOUT_LABEL, TIMEOUT_LABEL, building_time, TIMEOUT_LABEL]
                    )
                else:
                    m.append(
                        [TIMEOUT_LABEL, TIMEOUT_LABEL, TIMEOUT_LABEL, TIMEOUT_LABEL]
                    )
